"use client";

import { useState } from "react";
import { ethers } from "ethers";
import {
  getContract,
  getUSDCContract,
  CONTRACT_ADDRESS,
  PerfumeData,
} from "@/utils/contract";

export default function MintForm({ onMinted }: { onMinted: (tokenId: number, perfume: PerfumeData, desc: string) => void }) {
  const [gender, setGender] = useState(0);
  const [pType, setPType] = useState(2);
  const [loading, setLoading] = useState(false);
  const [step, setStep] = useState("");

  const mint = async () => {
    if (typeof window === "undefined" || !window.ethereum) {
      alert("Подключите MetaMask");
      return;
    }

    setLoading(true);
    setStep("Получение signer...");

    try {
      const provider = new ethers.BrowserProvider(window.ethereum);
      const signer = await provider.getSigner();
      const userAddress = await signer.getAddress();

      const contract = getContract(signer);
      const usdc = getUSDCContract(signer);

      const mintPrice = await contract.mintPrice();
      setStep("Проверка allowance USDC...");

      const allowance = await usdc.allowance(userAddress, CONTRACT_ADDRESS);
      if (allowance < mintPrice) {
        setStep("Approve USDC...");
        const tx = await usdc.approve(CONTRACT_ADDRESS, mintPrice * 100n);
        await tx.wait();
      }

      setStep("Минт парфюма...");
      const tx = await contract.createPerfume(gender, pType);
      const receipt = await tx.wait();

      // Парсим событие PerfumeCreated для получения tokenId
      const iface = contract.interface;
      let tokenId = 0;
      for (const log of receipt.logs) {
        try {
          const parsed = iface.parseLog({ topics: log.topics as string[], data: log.data });
          if (parsed && parsed.name === "PerfumeCreated") {
            tokenId = Number(parsed.args.tokenId);
            break;
          }
        } catch {}
      }

      if (tokenId === 0) throw new Error("TokenId not found in logs");

      setStep("Получение данных парфюма...");
      const perfume: PerfumeData = await contract.getPerfume(tokenId);

      setStep("Генерация ИИ-описания...");
      const desc = await generateDescription(perfume);

      onMinted(tokenId, perfume, desc);
      setStep("Готово!");
    } catch (error: any) {
      console.error(error);
      alert("Ошибка: " + (error.reason || error.message));
    } finally {
      setLoading(false);
      setStep("");
    }
  };

  async function generateDescription(perfume: PerfumeData): Promise<string> {
    const res = await fetch("/api/generate-description", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: perfume.name,
        gender: perfume.gender,
        pType: perfume.pType,
        topNotes: perfume.topNotes,
        heartNotes: perfume.heartNotes,
        baseNotes: perfume.baseNotes,
        concentration: perfume.concentration,
        rarity: perfume.rarity,
      }),
    });
    const data = await res.json();
    return data.description || "Описание недоступно";
  }

  return (
    <div className="glass-card p-8 max-w-md w-full">
      <h2 className="text-2xl font-bold mb-6 text-center">Создать аромат</h2>

      <div className="space-y-4 mb-6">
        <div>
          <label className="block text-sm text-white/60 mb-2">Пол</label>
          <div className="grid grid-cols-3 gap-2">
            {["Унисекс", "Мужской", "Женский"].map((g, i) => (
              <button
                key={i}
                onClick={() => setGender(i)}
                className={`py-2 rounded-lg text-sm font-medium transition-all ${
                  gender === i
                    ? "bg-arc-600 text-white"
                    : "bg-white/5 text-white/60 hover:bg-white/10"
                }`}
              >
                {g}
              </button>
            ))}
          </div>
        </div>

        <div>
          <label className="block text-sm text-white/60 mb-2">Тип</label>
          <div className="grid grid-cols-2 gap-2">
            {[
              { label: "Parfum", desc: "20-30%" },
              { label: "EDP", desc: "15-20%" },
              { label: "EDT", desc: "10-15%" },
              { label: "EDC", desc: "5-10%" },
            ].map((t, i) => (
              <button
                key={i}
                onClick={() => setPType(i)}
                className={`py-2 rounded-lg text-sm font-medium transition-all ${
                  pType === i
                    ? "bg-arc-600 text-white"
                    : "bg-white/5 text-white/60 hover:bg-white/10"
                }`}
              >
                <div>{t.label}</div>
                <div className="text-xs opacity-60">{t.desc}</div>
              </button>
            ))}
          </div>
        </div>
      </div>

      <button
        onClick={mint}
        disabled={loading}
        className="btn-primary w-full disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {loading ? step || "Обработка..." : "Создать за 10 USDC"}
      </button>

      <p className="text-xs text-white/40 text-center mt-4">
        Комиссия в USDC • Финализация &lt;1 сек
      </p>
    </div>
  );
}
