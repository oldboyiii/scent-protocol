...
// Standard ERC721 Enumerable + Custom getPerfume ABI
const NFT_ABI = [
...
export default function CollectionPage() {
  ...
  useEffect(() => {
    const fetchCollection = async () => {
      try {
        // 1. Get safe signer and user address
        const signer = await getArcSigner();
        const userAddress = await signer.getAddress();
        setAddress(userAddress);

        // 2. Create contract instance using provider for read-only calls
        const contract = new ethers.Contract(NFT_CONTRACT_ADDRESS, NFT_ABI, signer.provider);
        
        // 3. Get balance of owned tokens
        const balance = await contract.balanceOf(userAddress);
        const nfts: MyNFT[] = [];

        // 4. Fetch metadata for each owned token
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12 space-y-8 relative z-10">
      ...
      <p className="text-center text-white/50">
        {myNFTs.length} scent{myNFTs.length !== 1 ? "s" : ""} collected
      </p>
      ...
      {myNFTs.length === 0 ? (
        <div className="text-center text-white/40 py-20 glass-card-luxury rounded-2xl border border-white/10 p-8">
          ...
          <Link href="/mint" className="inline-block px-6 py-3 rounded-xl bg-amber-500 hover:bg-amber-400 text-slate-900 font-bold transition-colors">
          ...
          {myNFTs.map((nft) => {
            ...
            return (
              <div key={nft.tokenId} className={`group relative rounded-2xl p-6 space-y-4 backdrop-blur-xl bg-gradient-to-br ${style.bg} ${style.glow} border ${style.border} overflow-hidden transition-all duration-500 hover:scale-[1.02]`}>
                ...
                {hasFullData && nft.perfume && (
                  <>
                    <div className="relative flex flex-wrap gap-2 text-xs">
                      <span className="px-2 py-0.5 rounded-full bg-black/30 text-white/70 border border-white/10">{GENDER[nft.perfume.gender]}</span>
                      <span className="px-2 py-0.5 rounded-full bg-black/30 text-white/70 border border-white/10">{TYPE[nft.perfume.pType]}</span>
                ...
                {/* Action Button - Only visible here because we know user owns it */}
