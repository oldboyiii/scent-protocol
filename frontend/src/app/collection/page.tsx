    ...
    <div className="max-w-6xl mx-auto px-4 py-12 space-y-8 relative z-10">
      ...
      {myNFTs.length === 0 ? (
        <div className="text-center py-20 glass-card-luxury rounded-2xl border border-white/10 p-8">
          ...
          <Link href="/mint" className="px-6 py-3 rounded-xl bg-amber-500 text-slate-900 font-bold">Mint First Scent →</Link>
        </div>
      ) : (
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {myNFTs.map((nft) => {
            ...
            return (
              <div key={nft.tokenId} className={`group relative rounded-2xl p-6 backdrop-blur-xl bg-gradient-to-br ${style.bg} ${style.glow} border ${style.border} hover:scale-[1.02] transition-transform`}>
                <div className="flex justify-between mb-4">
                  <div><p className="text-xs text-white/40">Scent #{nft.tokenId}</p><h3 className="text-xl font-bold text-white">{nft.perfume?.name}</h3></div>
                  <span className={`text-xs font-bold px-2 py-1 rounded-full border backdrop-blur-md ${style.badge}`}>{["Common","Rare","Epic","Legendary"][rarity]}</span>
                </div>
                {nft.perfume && (
                  <>
                    <div className="flex gap-2 text-xs mb-4">
                      <span className="px-2 py-0.5 rounded-full bg-black/30 border border-white/10">{["Male","Female","Unisex"][nft.perfume.gender]}</span>
                      <span className="px-2 py-0.5 rounded-full bg-black/30 border border-white/10">{["Parfum","EDP","EDT","EDC"][nft.perfume.pType]}</span>
                    </div>
                    <div className="space-y-2 text-sm">
                      <div><span className="text-xs text-white/40">Top</span><div className="flex flex-wrap gap-1 mt-1">{nft.perfume.topNotes.map(n=><span key={n} className="px-2 py-0.5 rounded bg-black/30 text-amber-200 text-xs border border-amber-500/30">{n}</span>)}</div></div>
                      <div><span className="text-xs text-white/40">Heart</span><div className="flex flex-wrap gap-1 mt-1">{nft.perfume.heartNotes.map(n=><span key={n} className="px-2 py-0.5 rounded bg-black/30 text-rose-200 text-xs border border-rose-500/30">{n}</span>)}</div></div>
                      <div><span className="text-xs text-white/40">Base</span><div className="flex flex-wrap gap-1 mt-1">{nft.perfume.baseNotes.map(n=><span key={n} className="px-2 py-0.5 rounded bg-black/30 text-emerald-200 text-xs border border-emerald-500/30">{n}</span>)}</div></div>
                    </div>
                  </>
                )}
                <div className="mt-6 pt-4 border-t border-white/10">
                   <Link href={`/nft/${nft.tokenId}`} className="block w-full py-2 text-center bg-amber-500/20 text-amber-300 border border-amber-500/30 rounded-lg hover:bg-amber-500/30">Manage & Sell</Link>
                </div>
              </div>
            );
          })}
