...
// ABI только для получения данных, чтобы не ломать логику
const GENESIS_ABI = [
  ...
  useEffect(() => {
    ...
    async function fetch() {
      try {
        ...
        // ШАГ 1: Пробуем получить данные как раньше (ScentProtocol)
        try {
          const contract = getContract(provider);
          const data = await contract.getPerfume(id);
          setPerfume(data); // Всё сработало, выходим
          setLoading(false);
          return;
        } catch (e) {
          // Если здесь ошибка, значит этого ID нет в ScentProtocol. Идём дальше.
        }

        // ШАГ 2: Пробуем получить данные из Genesis
        try {
          ...
          // Приводим данные Genesis к ТОМУ ЖЕ формату, что и ScentProtocol, чтобы UI не сломался
          const formattedData = {
            name: data.name,
          ...
          setPerfume(formattedData);
          setLoading(false);
          return;
        } catch (e) {
          // Если и здесь ошибка, значит ID не существует нигде
        }
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-12 space-y-8 relative z-10">
      ...
      <div className={`group relative rounded-2xl p-8 backdrop-blur-xl bg-gradient-to-br ${style.bg} ${style.glow} border ${style.border} overflow-hidden transition-all duration-500`}>
        ...
        <div className="relative bg-black/30 rounded-lg p-4 text-white/70 italic border-l-2 border-white/10 mb-6">{description}</div>
